...
function mockClient(
  pages: Array<ReturnType<typeof makeReceiptPage> | ReturnType<typeof makeEmptyPage>>,
) {
  let call = 0;
  return {
...
describe('checkIdempotencyKey (RPC-level)', () => {
  ...
  it('returns receipt when memo matches on first page', async () => {
    const client = mockClient([
      makeReceiptPage({ key: 'my-key', objectId: '0xABC', previousTransaction: '0xTXDIGEST' }),
    ]);
    const result = await checkIdempotencyKey(
      client,
      SENDER,
  ...
  it('throws IDEMPOTENCY_CONFLICT when same key used with different recipient', async () => {
    const client = mockClient([
      makeReceiptPage({ key: 'dup-key', recipient: '0x' + 'c'.repeat(64) }),
    ]);
    await expect(
  ...
  it('throws IDEMPOTENCY_CONFLICT when same key used with different amount', async () => {
    const client = mockClient([
      makeReceiptPage({ key: 'dup-key', recipient: RECIPIENT, amount: '999' }),
    ]);
    try {
  ...
  it('returns receipt without conflict check when skipConflictCheck=true', async () => {
    ...
    const client = mockClient([
      makeReceiptPage({ key: 'skip-key', recipient: '0x' + 'f'.repeat(64), amount: '1' }),
    ]);
    const result = await checkIdempotencyKey(
      client,
      SENDER,
