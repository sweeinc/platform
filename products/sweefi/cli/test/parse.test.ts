...
describe('assertTxSuccess', () => {
  ...
  it('throws TX_OBJECT_CONFLICT for concurrent access (retryable)', () => {
    try {
      assertTxSuccess({
        effects: { status: { status: 'failure', error: 'SharedObjectLockContentious' } },
      });
      expect.unreachable('should have thrown');
    } catch (e) {
      expect(e).toBeInstanceOf(CliError);
  ...
  it('parses MoveAbort mandate expired (code 401) with requiresHumanAction', () => {
    try {
      assertTxSuccess({
        effects: { status: { status: 'failure', error: 'MoveAbort(mandate, 401)' } },
      });
      expect.unreachable('should have thrown');
    } catch (e) {
      expect(e).toBeInstanceOf(CliError);
  ...
  it('parses MoveAbort prepaid max calls exceeded (code 606)', () => {
    try {
      assertTxSuccess({
        effects: { status: { status: 'failure', error: 'MoveAbort(prepaid, 606)' } },
      });
      expect.unreachable('should have thrown');
    } catch (e) {
      expect(e).toBeInstanceOf(CliError);
