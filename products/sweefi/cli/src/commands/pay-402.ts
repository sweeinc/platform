...
import { decodePaymentRequired, S402_HEADERS } from 's402';
import { createS402Client } from '@sweefi/sui';
import { CliError, debug, requireSigner } from '../context.js';
import { buildIdempotencyMemo, checkIdempotencyKey } from '../idempotency.js';
import { explorerUrl, formatBalance, outputSuccess } from '../output.js';
...
export async function pay402(
  ...
  if (!flags.url) {
    throw new CliError(
      'MISSING_ARGS',
      'Usage: sweefi pay-402 --url <URL>',
      false,
      'Example: sweefi pay-402 --url https://api.example.com/premium/data',
    );
  }
    ...
    if (existing) {
      ...
      outputSuccess(
        'pay-402',
        {
          ...
          message:
            'Payment already exists with this idempotency key. No new payment created. Re-fetch the URL directly if you need the content.',
          network: ctx.network,
        },
        flags.human ?? false,
        reqCtx,
  ...
  try {
    ...
    if (e instanceof DOMException && e.name === 'AbortError') {
      throw new CliError(
        ...
        true,
        'Increase --timeout or check the URL is reachable',
      );
    }
    throw new CliError(
      'NETWORK_ERROR',
      `Failed to reach ${flags.url}: ${e instanceof Error ? e.message : String(e)}`,
      true,
  ...
  const memo = flags.idempotencyKey ? buildIdempotencyMemo(flags.idempotencyKey) : undefined;
  ...
  try {
    ...
    if (e instanceof DOMException && e.name === 'AbortError') {
      ...
      throw new CliError(
        'TIMEOUT',
        `Paid request timed out after ${ctx.timeoutMs}ms`,
        false,
        'The payment may have been submitted — check your balance before retrying. Increase --timeout for slow endpoints.',
      );
    }
    // PAYMENT_FAILED is also not safely retryable — the TX may have been signed and broadcast
    throw new CliError(
      'PAYMENT_FAILED',
      `Paid fetch failed: ${e instanceof Error ? e.message : String(e)}`,
      false,
      'The payment may have been submitted — check your balance before retrying',
    );
  }
  clearTimeout(paidTimer);
  ...
  const data: Record<string, unknown> = {
    ...
    requirements,
    body:
      responseBody.length > 10_000
        ? responseBody.slice(0, 10_000) + '...(truncated)'
        : responseBody,
    network: ctx.network,
