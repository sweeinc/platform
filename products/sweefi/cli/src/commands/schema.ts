  ...
  return {
    ...
    globalFlags: [
      {
        ...
        type: 'string',
        default: 'testnet',
        description: 'Sui network (testnet, mainnet, devnet). Overrides SUI_NETWORK env var.',
      },
      {
        name: 'human',
        type: 'boolean',
        default: false,
        description: 'Human-readable output instead of JSON envelope',
      },
      {
        name: 'json',
        type: 'boolean',
        default: false,
        description: 'Force JSON output (overrides --human)',
      },
      {
        name: 'timeout',
        type: 'number',
    ...
    envVars: [
      {
        ...
        required: true,
        description: 'Wallet key (bech32 suiprivkey1... or base64). Required for any transaction.',
      },
      {
        name: 'SUI_NETWORK',
        required: false,
        description: 'Default network: testnet | mainnet | devnet',
      },
      {
        name: 'SUI_RPC_URL',
        required: false,
        description: 'Custom RPC endpoint (overrides default for network)',
      },
      {
        name: 'SUI_PACKAGE_ID',
        required: false,
        description: 'Override deployed contract package ID',
      },
    ],
    commands: [
      {
        ...
        args: [
          { name: 'recipient', type: 'string', required: true, description: 'Sui address (0x...)' },
          {
            ...
            type: 'number',
            required: true,
            description: 'Amount in display units (e.g., 1.5 = 1.5 SUI)',
          },
        ],
        flags: [
          {
            ...
            type: 'string',
            default: 'SUI',
            description: 'Token to pay with (SUI, USDC, USDT)',
          },
          {
            name: 'idempotency-key',
            type: 'string',
            required: false,
            requiredWhen: 'JSON output mode (default). Auto-generated in --human mode.',
            description: 'UUID for dedup — prevents double-spend on crash/retry',
          },
          { name: 'memo', type: 'string', description: 'On-chain memo text' },
          {
            name: 'mandate',
            type: 'string',
            description: 'Mandate object ID for delegated payments',
          },
          {
            name: 'registry',
            type: 'string',
            description: 'Registry object ID (required with --mandate)',
          },
          {
            name: 'dry-run',
            type: 'boolean',
            default: false,
            description: 'Simulate without executing — includes affordability check',
          },
        ],
      },
      {
        ...
        flags: [
          ...
          { name: 'body', type: 'string', description: 'Request body' },
          {
            name: 'header',
            type: 'string[]',
            description: 'Request headers (K: V format, repeatable)',
          },
          {
            name: 'idempotency-key',
            type: 'string',
          ...
          },
          {
            ...
            type: 'boolean',
            default: false,
            description: 'Probe URL and show payment requirements without executing payment',
          },
        ],
      },
      {
        ...
        flags: [{ name: 'coin', type: 'string', default: 'SUI', description: 'Token type' }],
      },
      {
        name: 'receipt',
        ...
        args: [
          {
            ...
            type: 'string',
            required: true,
            description: 'PaymentReceipt object ID',
          },
        ],
        flags: [],
      },
      {
        ...
        args: [
          { name: 'provider', type: 'string', required: true, description: 'Provider Sui address' },
          {
            ...
            type: 'number',
            required: true,
            description: 'Deposit amount in display units',
          },
        ],
        flags: [
          ...
          { name: 'rate', type: 'string', description: 'Rate per call in base units' },
          {
            name: 'max-calls',
            type: 'string',
            description: 'Maximum call count (default: unlimited)',
          },
          {
            name: 'dry-run',
            type: 'boolean',
        ...
        args: [
          {
            ...
            type: 'string',
            required: true,
            description: 'PrepaidBalance object ID',
          },
        ],
        flags: [],
      },
      ...
      },
      {
        ...
        args: [
          { name: 'agent', type: 'string', required: true, description: 'Agent Sui address' },
          {
            ...
            type: 'number',
            required: true,
            description: 'Per-transaction cap in display units',
          },
          {
            name: 'max-total',
            type: 'number',
            required: true,
            description: 'Lifetime spending cap in display units',
          },
          {
            name: 'expires-in',
            type: 'string',
            required: true,
            description: 'Duration (e.g., 7d, 24h, 30m)',
          },
        ],
        flags: [
          { name: 'coin', type: 'string', default: 'SUI', description: 'Token type' },
    ...
    errorCodes: [
      ...
      { code: 'MISSING_ARGS', retryable: false, description: 'Required arguments not provided' },
      {
        code: 'INVALID_AMOUNT',
        retryable: false,
        description: 'Amount is not a valid positive number',
      },
      {
        code: 'INVALID_ADDRESS',
        retryable: false,
        description: 'Not a valid Sui address (0x + 64 hex chars)',
      },
      { code: 'INVALID_OBJECT_ID', retryable: false, description: 'Not a valid Sui object ID' },
      {
        code: 'INVALID_DURATION',
        retryable: false,
        description: 'Duration format invalid (use 7d, 24h, 30m)',
      },
      { code: 'INVALID_VALUE', retryable: false, description: 'Flag value is invalid' },
      {
        code: 'INVALID_URL',
        retryable: false,
        description: 'URL is malformed or uses unsupported scheme',
      },
      {
        code: 'INVALID_NETWORK',
        retryable: false,
        description: 'Unknown network (use testnet, mainnet, devnet)',
      },
      {
        code: 'INVALID_MANDATE',
        retryable: false,
        description: 'Mandate parameters are inconsistent',
      },
      // Auth / wallet
      { code: 'NO_WALLET', retryable: false, description: 'SUI_PRIVATE_KEY not set or unreadable' },
      {
        code: 'NO_ADDRESS',
        retryable: false,
        description: 'No address provided and no wallet configured',
      },
      // Idempotency
      {
        code: 'IDEMPOTENCY_KEY_REQUIRED',
        retryable: false,
        description: 'JSON mode requires --idempotency-key for pay and pay-402',
      },
      {
        code: 'IDEMPOTENCY_CONFLICT',
        retryable: false,
        description: 'Key already used with different recipient/amount',
      },
      ...
      { code: 'TX_OUT_OF_GAS', retryable: true, description: 'Transaction ran out of gas' },
      {
        ...
        retryable: true,
        description: 'Concurrent object access (transient)',
      },
      ...
      // Move abort codes (on-chain contract errors)
      {
        ...
        retryable: false,
        description: 'Payment amount less than required',
      },
      { code: 'ZERO_AMOUNT', retryable: false, description: 'Amount cannot be zero' },
      {
        code: 'MANDATE_NOT_DELEGATE',
        retryable: false,
        description: 'Signer is not the authorized delegate',
      },
      {
      ...
      },
      {
        ...
        retryable: false,
        description: 'Amount exceeds per-transaction cap',
      },
      {
        code: 'MANDATE_TOTAL_EXCEEDED',
        retryable: false,
        requiresHuman: true,
        description: 'Lifetime spending cap reached',
      },
      {
        code: 'MANDATE_REVOKED',
        retryable: false,
        requiresHuman: true,
        description: 'Mandate revoked by delegator',
      },
      {
        code: 'PREPAID_NOT_AGENT',
        retryable: false,
        description: 'Signer is not the agent for this balance',
      },
      {
        code: 'PREPAID_NOT_PROVIDER',
        retryable: false,
        description: 'Signer is not the authorized provider',
      },
      {
        code: 'PREPAID_WITHDRAWAL_LOCKED',
        retryable: true,
        description: 'Withdrawal grace period active',
      },
      {
        code: 'PREPAID_MAX_CALLS_EXCEEDED',
        retryable: false,
        description: 'Maximum call count exceeded',
      },
      {
        code: 'PREPAID_RATE_EXCEEDED',
        retryable: false,
        description: 'Claim exceeds available balance',
      },
      // Network
      ...
      // pay-402 specific
      {
        code: 'UNKNOWN_402',
        retryable: false,
        description: 'Server returned 402 without s402 headers',
      },
      {
        code: 'PAYMENT_FAILED',
        retryable: false,
        description:
          's402 paid fetch failed — payment may have been submitted; check balance before retrying',
      },
      // System
