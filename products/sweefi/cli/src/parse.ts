...
export function parseAmount(value: string, coinType: string): bigint {
  if (!value || value.trim() === '') {
    throw new CliError(
      ...
      'Amount is required',
      false,
      "Provide a positive number like '1.5'",
    );
  }
  ...
  if (/[eE]/.test(trimmed) || /^[+-]?Infinity$/.test(trimmed) || /^0[xX]/.test(trimmed)) {
    throw new CliError(
      ...
      false,
      "Use a plain number like '1.5' or '1500000000'",
    );
  }
  ...
  if (Number.isNaN(num) || !Number.isFinite(num) || num <= 0) {
    throw new CliError(
      ...
      `Invalid amount: "${value}"`,
      false,
      "Provide a positive number like '1.5' or '100'",
    );
  }
  ...
  if (result === 0n) {
    throw new CliError(
      'INVALID_AMOUNT',
      `Amount "${value}" is too small (${decimals} decimal places max)`,
      false,
      `Minimum is 0.${'0'.repeat(decimals - 1)}1`,
    );
  }
  return result;
}
...
export function parseDuration(value: string): bigint {
  ...
  if (!match) {
    throw new CliError(
      ...
      false,
      'Use formats like "7d", "24h", "30m", or "3600000"',
    );
  }
...
export function validateObjectId(input: string, label: string): string {
  if (!/^0x[a-fA-F0-9]+$/.test(input)) {
    throw new CliError(
      'INVALID_OBJECT_ID',
      `${label} is not a valid object ID: "${input}"`,
      false,
      'Object IDs start with 0x followed by hex characters',
    );
  }
  return input;
}
...
> = {
  ...
  400: {
    ...
    action:
      "Run 'sweefi mandate check <mandateId>' to verify the delegate address matches your wallet",
    retryable: false,
    humanRequired: false,
  },
  401: {
  ...
  402: {
    ...
    action:
      "Run 'sweefi mandate check <mandateId>' to see the per-tx cap, then reduce the amount below it. If the full amount is required, inform the user and wait for them to raise the cap.",
    retryable: false,
    humanRequired: false,
  },
  403: {
  ...
  604: {
    ...
    action: 'Wait for the withdrawal delay to pass, then retry',
    retryable: true,
    humanRequired: false,
  },
  606: {
    ...
    action:
      "Create a new prepaid balance with 'sweefi prepaid deposit' and a higher --max-calls value",
    retryable: false,
    humanRequired: false,
  },
  607: {
    code: 'PREPAID_RATE_EXCEEDED',
    message: 'Claim amount exceeds available balance',
    action:
      "Check remaining balance with 'sweefi prepaid status <balanceId>'. Top up or create a new prepaid deposit.",
    retryable: false,
    humanRequired: false,
  },
};
...
export function assertTxSuccess(result: {
  ...
  if (status !== 'success') {
    ...
    if (/InsufficientGas|out of gas/i.test(error)) {
      throw new CliError(
        'TX_OUT_OF_GAS',
        `Transaction ran out of gas: ${error}`,
        true,
        'Retry with a higher gas budget',
      );
    }
    if (
      /ObjectVersionUnavailableForConsumption|SharedObjectLockContentious|concurrency/i.test(error)
    ) {
      throw new CliError(
        'TX_OBJECT_CONFLICT',
        `Concurrent object access: ${error}`,
        true,
