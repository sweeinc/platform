...
function formatPay(data: Record<string, unknown>): string {
  ...
  if (data.dryRun) {
    ...
    return [
      ...
      section([
        ...
        [
          'Gas (est)',
          data.estimatedGasMist !== undefined ? `${data.estimatedGasMist} MIST` : undefined,
        ],
        ['Affordable', status],
        ...(balance
          ? Object.entries(balance).map(([k, v]) => [`Balance (${k})`, v] as [string, string])
          : []),
        ...(shortfall
          ? Object.entries(shortfall).map(
              ([k, v]) => [`Shortfall (${k})`, red(v)] as [string, string],
            )
          : []),
      ]),
  ...
  return [
    ...
    section([
      ...
      ...(data.idempotencyKey
        ? [['Idempotency', data.idempotencyKey as string] as [string, string]]
        : []),
...
function formatDoctor(data: Record<string, unknown>): string {
  ...
  for (const check of checks) {
    ...
    lines.push(
      `  ${icon} ${check.name.padEnd(maxName + 2)}${check.value.padEnd(maxValue + 2)}${detail}`,
    );
  }
...
function formatReceipt(data: Record<string, unknown>): string {
  ...
  return [
    ...
    ...(fields && Object.keys(fields).length > 0
      ? [section(Object.entries(fields).map(([k, v]) => [k, String(v)] as [string, string]))]
      : []),
    '',
  ].join('\n');
}
...
function formatMandateCheck(data: Record<string, unknown>): string {
  const { mandateId, network, owner: _owner, expired, ...fields } = data;
  const expiredStatus =
    expired === true ? red('EXPIRED') : expired === false ? green('ACTIVE') : dim('unknown');
  return [
