...
describe('routes', () => {
  ...
  describe('POST /verify', () => {
    it('delegates to facilitator.verify()', async () => {
      ...
      expect(facilitator.verify).toHaveBeenCalledWith(VALID_PAYLOAD, VALID_REQUIREMENTS);
    });
  ...
  describe('POST /settle', () => {
    it('delegates to facilitator.settle()', async () => {
      ...
      expect(facilitator.settle).toHaveBeenCalledWith(VALID_PAYLOAD, VALID_REQUIREMENTS);
    });
  ...
  describe('POST /s402/process', () => {
    it('delegates to facilitator.process() for atomic verify+settle', async () => {
      ...
      expect(body.success).toBe(true);
      expect(body.txDigest).toBe('mock-tx-digest');
      expect(facilitator.process).toHaveBeenCalledWith(VALID_PAYLOAD, VALID_REQUIREMENTS);
    });
  });
  ...
  describe('GET /settlements', () => {
    ...
    it('returns only records for the calling API key — not other keys', async () => {
      ...
      expect(body.records.map((r: { txDigest: string }) => r.txDigest)).toEqual([
        'digest-a1',
        'digest-a2',
      ]);
      // key-b records must not appear
      expect(
        body.records.find((r: { txDigest: string }) => r.txDigest === 'digest-b1'),
      ).toBeUndefined();
    });
