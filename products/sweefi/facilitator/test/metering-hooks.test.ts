...
import { meteringContext, recordSettlement, requestApiKey } from '../src/metering/hooks';
...
describe('recordSettlement', () => {
  ...
  it('handles multiple settlements per API key', () => {
    recordSettlement(
      ...
      'multi-key',
      makeRequirements({ amount: '1000000' }),
      makeSettleResult({ txDigest: 'tx-1' }),
      10_000,
    );
    recordSettlement(
      tracker,
      'multi-key',
      makeRequirements({ amount: '2000000' }),
      makeSettleResult({ txDigest: 'tx-2' }),
      10_000,
    );
  ...
  it('isolates settlements across different API keys', () => {
    ...
    recordSettlement(
      tracker,
      'key-b',
      makeRequirements({ amount: '3000000' }),
      makeSettleResult(),
      5_000,
    );
