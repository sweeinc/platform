...
function createMockGasSponsorService(
  overrides: Partial<{
    initialized: boolean;
    address: string;
...
describe('Dedup cache security', () => {
  function makeTestApp(apiKey: string, ttlMs = 60_000, maxEntries = 10_000) {
    ...
    const handler = vi
      .fn()
      .mockImplementation((c: any) => c.json({ success: true, txDigest: 'mock-digest' }));
    app.use('*', async (c, next) => {
  ...
  it('passes through to handler when no apiKey is set (unauthenticated)', async () => {
    const app = new Hono();
    const handler = vi.fn().mockImplementation((c: any) => c.json({ success: true }));
  ...
  it('malformed JSON passes through to handler (not cached)', async () => {
    const app = new Hono();
    const handler = vi.fn().mockImplementation((c: any) => c.json({ error: 'bad json' }, 400));
    app.use('*', async (c, next) => {
...
describe('F-18: Error responses do not leak internal state', () => {
  it('/verify returns generic error on unexpected exception', async () => {
    ...
    const res = await app.request('/verify', {
      ...
      body: JSON.stringify({
        ...
        paymentRequirements: VALID_REQUIREMENTS,
      }),
    });
  ...
  it('/settle returns generic error on unexpected exception', async () => {
    ...
    facilitator.settle.mockRejectedValue(
      new Error('Sui RPC: method not found, endpoint: https://internal-rpc.myco.io'),
    );
    const tracker = new UsageTracker();
    const app = createRoutes(facilitator as any, tracker, 50);
  ...
  it('/s402/process returns generic error on unexpected exception', async () => {
    const facilitator = createMockFacilitator();
    facilitator.process.mockRejectedValue(
      new Error('Database connection pool exhausted: postgres://admin:s3cret@db.internal:5432'),
    );
    const tracker = new UsageTracker();
    const app = createRoutes(facilitator as any, tracker, 50);
  ...
  it('/sponsor returns generic error for non-pool-exhaustion failures', async () => {
    ...
    const gasSponsorService = createMockGasSponsorService({
      sponsor: vi
        .fn()
        .mockRejectedValue(new Error('Signer private key mismatch at /etc/secrets/key.pem')),
    });
    const app = createRoutes(
      facilitator as any,
      tracker,
