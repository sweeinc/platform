...
function captureHandlers(server: McpServer, ctx: SweefiContext) {
  ...
  const spy = vi
    ...
    .mockImplementation((name: string, config: any, cb: any) => {
...
describe('admin tools', () => {
  ...
  describe('sweefi_pause_protocol handler', () => {
    it('throws when no signer configured', async () => {
      ...
      await expect(handler({ adminCapId: ADMIN_CAP_ID })).rejects.toThrow('Wallet not configured');
    });
    ...
    it('throws on transaction failure', async () => {
      ...
      await expect(handler({ adminCapId: ADMIN_CAP_ID })).rejects.toThrow();
    });
  });
  ...
  describe('sweefi_burn_admin_cap handler', () => {
    ...
    it('throws on transaction failure', async () => {
      ...
      await expect(handler({ adminCapId: ADMIN_CAP_ID })).rejects.toThrow('TX_FAILED');
    });
