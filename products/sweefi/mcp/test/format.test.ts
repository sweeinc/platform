...
describe('parseAmount', () => {
  ...
  it('uses custom field name in error messages', () => {
    expect(() => parseAmount('abc', 'depositAmount')).toThrow(
      'depositAmount must be a non-negative integer',
    );
  });
...
describe('parseAmountOrZero', () => {
  ...
  it('uses custom field name in errors', () => {
    expect(() => parseAmountOrZero('abc', 'dailyLimit')).toThrow(
      'dailyLimit must be a non-negative integer',
    );
  });
});
...
describe('assertTxSuccess', () => {
  ...
  it('throws with unknown error when no error message', () => {
    expect(() => assertTxSuccess({ effects: { status: { status: 'failure' } } })).toThrow(
      'Transaction failed on-chain: Unknown error',
    );
  });
  ...
  it('parses MoveAbort mandate revoked (code 405) with structured JSON', () => {
    try {
      assertTxSuccess({
        effects: { status: { status: 'failure', error: 'MoveAbort(mandate, 405)' } },
      });
  ...
  it('parses MoveAbort mandate per-tx exceeded (code 402) as agent-recoverable', () => {
    try {
      assertTxSuccess({
        effects: { status: { status: 'failure', error: 'MoveAbort(mandate, 402)' } },
      });
      expect.unreachable('should have thrown');
    } catch (e) {
      const msg = (e as Error).message;
  ...
  it('falls back to TX_FAILED with JSON for unknown abort codes', () => {
    try {
      assertTxSuccess({
        effects: { status: { status: 'failure', error: 'MoveAbort(custom, 999)' } },
      });
      expect.unreachable('should have thrown');
    } catch (e) {
      const msg = (e as Error).message;
  ...
  it('parses stream abort code (106 = nothing to claim) as retryable', () => {
    try {
      assertTxSuccess({
        effects: { status: { status: 'failure', error: 'MoveAbort(stream, 106)' } },
      });
      expect.unreachable('should have thrown');
    } catch (e) {
      const msg = (e as Error).message;
  ...
  it('parses escrow abort code (206 = already resolved) as non-retryable', () => {
    try {
      assertTxSuccess({
        effects: { status: { status: 'failure', error: 'MoveAbort(escrow, 206)' } },
      });
      expect.unreachable('should have thrown');
    } catch (e) {
      const msg = (e as Error).message;
  ...
  it('parses prepaid abort code (604 = withdrawal locked) as retryable', () => {
    try {
      assertTxSuccess({
        effects: { status: { status: 'failure', error: 'MoveAbort(prepaid, 604)' } },
      });
      expect.unreachable('should have thrown');
    } catch (e) {
      const msg = (e as Error).message;
  ...
  it('parses agent_mandate abort code (559 = level not authorized) as human-required', () => {
    try {
      assertTxSuccess({
        effects: { status: { status: 'failure', error: 'MoveAbort(agent_mandate, 559)' } },
      });
      expect.unreachable('should have thrown');
    } catch (e) {
      const msg = (e as Error).message;
