...
export function createContext(config?: SweefiMcpConfig): SweefiContext {
  ...
  const suiClient = rpcUrl
    ...
    : new SuiJsonRpcClient({
        url: getJsonRpcFullnodeUrl(suiNetwork as 'testnet' | 'mainnet' | 'devnet'),
        network: suiNetwork,
      });
  ...
  if (privateKey) {
    try {
      ...
      if (scheme === 'ED25519') {
        ...
        console.warn(
          `[sweefi-mcp] Key scheme "${scheme}" is not supported. Only ED25519 keys work. Transaction tools will be disabled.`,
        );
      }
    } catch {
