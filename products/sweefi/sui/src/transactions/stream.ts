...
export class StreamContract {
  ...
  create =
    (params: CreateStreamParams): ((tx: Transaction) => void) =>
    (tx: Transaction) => {
  ...
  createWithTimeout =
    (params: CreateStreamWithTimeoutParams): ((tx: Transaction) => void) =>
    (tx: Transaction) => {
  ...
  claim =
    (params: StreamOpParams): ((tx: Transaction) => void) =>
    (tx: Transaction) => {
      tx.moveCall({
        target: `${this.#config.packageId}::stream::claim`,
        typeArguments: [params.coinType],
        arguments: [tx.object(params.meterId), tx.object(this.#config.SUI_CLOCK)],
      });
    };

  batchClaim =
    (params: BatchClaimParams): ((tx: Transaction) => void) =>
    (tx: Transaction) => {
      ...
      if (params.meterIds.length > 512) {
        throw new Error(
          'StreamContract.batchClaim: max 512 streams per batch (Sui PTB command limit)',
        );
      }
      ...
      for (const meterId of params.meterIds) {
        tx.moveCall({
          target: `${this.#config.packageId}::stream::claim`,
          typeArguments: [params.coinType],
          arguments: [tx.object(meterId), tx.object(this.#config.SUI_CLOCK)],
        });
      }
    };
  ...
  topUp =
    (params: StreamTopUpParams): ((tx: Transaction) => void) =>
    (tx: Transaction) => {
