...
import type { CreateUptoDepositParams, ExpireUptoParams, SettleUptoParams } from '../ptb/upto.js';
...
export class UptoContract {
  ...
  create =
    (params: CreateUptoDepositParams): ((tx: Transaction) => void) =>
    (tx: Transaction) => {
  ...
  settle =
    (params: SettleUptoParams): ((tx: Transaction) => void) =>
    (tx: Transaction) => {
  ...
  expire =
    (params: ExpireUptoParams): ((tx: Transaction) => void) =>
    (tx: Transaction) => {
      tx.moveCall({
        target: `${this.#config.packageId}::upto_deposit::expire`,
        typeArguments: [params.coinType],
        arguments: [tx.object(params.depositId), tx.object(this.#config.SUI_CLOCK)],
      });
    };
}
