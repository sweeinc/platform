...
import type { CreateInvoiceParams, PayInvoiceParams, PayParams } from '../ptb/types.js';
...
export class PaymentContract {
   ...
   */
  pay =
    (params: PayParams): ((tx: Transaction) => void) =>
    (tx: Transaction) => {
   ...
   */
  payComposable =
    (params: PayParams): ((tx: Transaction) => TransactionResult) =>
    (tx: Transaction) => {
   ...
   */
  createInvoice =
    (params: CreateInvoiceParams): ((tx: Transaction) => void) =>
    (tx: Transaction) => {
   ...
   */
  payInvoice =
    (params: PayInvoiceParams): ((tx: Transaction) => void) =>
    (tx: Transaction) => {
      ...
      tx.moveCall({
        ...
        typeArguments: [params.coinType],
        arguments: [tx.object(params.invoiceId), coin, tx.object(this.#config.SUI_CLOCK)],
      });
    };
}
