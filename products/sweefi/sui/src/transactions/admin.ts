...
export class AdminContract {
  ...
  pause =
    (params: AdminParams): ((tx: Transaction) => void) =>
    (tx: Transaction) => {
  ...
  unpause =
    ...
    (tx: Transaction) => {
      ...
      tx.moveCall({
        ...
        arguments: [tx.object(params.adminCapId), tx.object(protocolState)],
      });
    };

  autoUnpause =
    (_params: AutoUnpauseParams): ((tx: Transaction) => void) =>
    (tx: Transaction) => {
      const protocolState = this.#config.requireProtocolState();

      tx.moveCall({
        target: `${this.#config.packageId}::admin::auto_unpause`,
        arguments: [tx.object(protocolState), tx.object(this.#config.SUI_CLOCK)],
      });
    };
