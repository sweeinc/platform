...
export class EscrowContract {
  ...
  create =
    (params: CreateEscrowParams): ((tx: Transaction) => void) =>
    (tx: Transaction) => {
  ...
  release =
    (params: EscrowOpParams): ((tx: Transaction) => void) =>
    (tx: Transaction) => {
      tx.moveCall({
        target: `${this.#config.packageId}::escrow::release_and_keep`,
        typeArguments: [params.coinType],
        arguments: [tx.object(params.escrowId), tx.object(this.#config.SUI_CLOCK)],
      });
    };

  releaseComposable =
    (params: EscrowOpParams): ((tx: Transaction) => TransactionResult) =>
    (tx: Transaction) => {
      return tx.moveCall({
