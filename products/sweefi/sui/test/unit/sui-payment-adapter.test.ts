...
function makeMockDryRunSuccess(
...
function makeAdapter(
  ...
  const mockClient = {
    dryRunTransactionBlock: vi.fn().mockResolvedValue(dryRunResult ?? makeMockDryRunSuccess()),
...
describe('SuiPaymentAdapter', () => {
  ...
  describe('simulate()', () => {
    ...
    it('returns INSUFFICIENT_BALANCE when dry-run reports insufficient funds', async () => {
      const { adapter } = makeAdapter(
        makeMockDryRunFailure('Insufficient balance for coin type 0x2::sui::SUI'),
      );
  ...
  describe('signAndBroadcast()', () => {
    ...
    it('propagates client errors', async () => {
      const { adapter, mockClient } = makeAdapter();
      (mockClient.executeTransactionBlock as Mock).mockRejectedValue(new Error('Network timeout'));
