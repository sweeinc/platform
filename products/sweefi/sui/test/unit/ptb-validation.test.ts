...
import { buildCreateMandateTx, buildMandatedPayTx } from '../../src/ptb/mandate';
import {
  buildDepositTx,
...
describe('assertFeeMicroPercent', () => {
  ...
  it('rejects MAX_SAFE_INTEGER + 1 (not integer-safe)', () => {
    ...
    expect(() => assertFeeMicroPercent(Number.MAX_SAFE_INTEGER + 1, 'test')).toThrow(
      'feeMicroPercent',
    );
  });
});
...
describe('buildPayTx validation', () => {
  ...
  it('rejects feeMicroPercent = -1', () => {
    expect(() => buildPayTx(config, { ...validParams, amount: 100n, feeMicroPercent: -1 })).toThrow(
      'feeMicroPercent',
    );
  });
  ...
  it('rejects feeMicroPercent = 1_000_001', () => {
    expect(() =>
      buildPayTx(config, { ...validParams, amount: 100n, feeMicroPercent: 1_000_001 }),
    ).toThrow('feeMicroPercent');
  });

  it('rejects feeMicroPercent = NaN', () => {
    expect(() =>
      buildPayTx(config, { ...validParams, amount: 100n, feeMicroPercent: NaN }),
    ).toThrow('feeMicroPercent');
  });
});
...
describe('buildPayComposableTx validation', () => {
  ...
  it('rejects amount = 0n', () => {
    expect(() => buildPayComposableTx(config, { ...validParams, amount: 0n })).toThrow(
      'must be > 0',
    );
  });
  ...
  it('rejects feeMicroPercent = -1', () => {
    expect(() =>
      buildPayComposableTx(config, { ...validParams, amount: 100n, feeMicroPercent: -1 }),
    ).toThrow('feeMicroPercent');
  });
});
...
describe('buildCreateInvoiceTx validation', () => {
  ...
  it('rejects expectedAmount = 0n', () => {
    expect(() => buildCreateInvoiceTx(config, { ...validParams, expectedAmount: 0n })).toThrow(
      'must be > 0',
    );
  });

  it('rejects negative expectedAmount', () => {
    expect(() => buildCreateInvoiceTx(config, { ...validParams, expectedAmount: -100n })).toThrow(
      'must be > 0',
    );
  });
...
describe('buildPayAndProveTx validation', () => {
  ...
  it('rejects feeMicroPercent = -1', () => {
    expect(() =>
      buildPayAndProveTx(config, { ...validParams, amount: 100n, feeMicroPercent: -1 }),
    ).toThrow('feeMicroPercent');
  });
});
...
describe('buildCreateStreamTx validation', () => {
  ...
  it('rejects depositAmount = 0n', () => {
    expect(() => buildCreateStreamTx(fullConfig, { ...validParams, depositAmount: 0n })).toThrow(
      'must be > 0',
    );
  });
  ...
  it('rejects ratePerSecond = 0n', () => {
    expect(() =>
      buildCreateStreamTx(fullConfig, { ...validParams, depositAmount: 100n, ratePerSecond: 0n }),
    ).toThrow('must be > 0');
  });

  it('rejects feeMicroPercent = -1', () => {
    expect(() =>
      buildCreateStreamTx(fullConfig, { ...validParams, depositAmount: 100n, feeMicroPercent: -1 }),
    ).toThrow('feeMicroPercent');
  });
  ...
  it('FINDING F-02: does NOT validate budgetCap = 0n (missing assertPositive)', () => {
    ...
    expect(() =>
      buildCreateStreamTx(fullConfig, { ...validParams, depositAmount: 100n, budgetCap: 0n }),
    ).not.toThrow();
  });
});
...
describe('buildCreateStreamWithTimeoutTx validation', () => {
  ...
  it('rejects depositAmount = 0n', () => {
    expect(() =>
      buildCreateStreamWithTimeoutTx(fullConfig, { ...validParams, depositAmount: 0n }),
    ).toThrow('must be > 0');
  });

  it('rejects ratePerSecond = 0n', () => {
    expect(() =>
      buildCreateStreamWithTimeoutTx(fullConfig, { ...validParams, ratePerSecond: 0n }),
    ).toThrow('must be > 0');
  });
  ...
  it('FINDING F-03: does NOT validate recipientCloseTimeoutMs = 0n (missing min check)', () => {
    expect(() =>
      buildCreateStreamWithTimeoutTx(fullConfig, { ...validParams, recipientCloseTimeoutMs: 0n }),
    ).not.toThrow();
  });
});
...
describe('buildTopUpTx (stream) validation', () => {
  ...
  it('FIXED F-04: validates depositAmount = 0n (assertPositive added)', () => {
    expect(() => buildTopUpTx(fullConfig, { ...validParams, depositAmount: 0n })).toThrow(
      /must be > 0/,
    );
  });

  it('FIXED F-04: validates negative depositAmount', () => {
    expect(() => buildTopUpTx(fullConfig, { ...validParams, depositAmount: -1n })).toThrow(
      /must be > 0/,
    );
  });
});
...
describe('buildCreateEscrowTx validation', () => {
  ...
  it('rejects depositAmount = 0n', () => {
    expect(() => buildCreateEscrowTx(fullConfig, { ...validParams, depositAmount: 0n })).toThrow(
      'must be > 0',
    );
  });

  it('rejects negative depositAmount', () => {
    expect(() => buildCreateEscrowTx(fullConfig, { ...validParams, depositAmount: -1n })).toThrow(
      'must be > 0',
    );
  });
  ...
  it('FINDING F-05: does NOT validate deadlineMs = 0n (past deadline)', () => {
    ...
    expect(() =>
      buildCreateEscrowTx(fullConfig, { ...validParams, depositAmount: 100n, deadlineMs: 0n }),
    ).not.toThrow();
  });
});
...
describe('buildDepositTx (prepaid) validation', () => {
  ...
  it('rejects ratePerCall = 0n', () => {
    expect(() =>
      buildDepositTx(fullConfig, { ...validParams, amount: 100n, ratePerCall: 0n }),
    ).toThrow('must be > 0');
  });

  it('rejects feeMicroPercent = NaN', () => {
    expect(() =>
      buildDepositTx(fullConfig, { ...validParams, amount: 100n, feeMicroPercent: NaN }),
    ).toThrow('feeMicroPercent');
  });
  ...
  it('FINDING F-06: does NOT validate withdrawalDelayMs = 0n (below Move minimum)', () => {
    expect(() =>
      buildDepositTx(fullConfig, { ...validParams, amount: 100n, withdrawalDelayMs: 0n }),
    ).not.toThrow();
  });
});
...
describe('buildDepositWithReceiptsTx validation', () => {
  ...
  it('rejects amount = 0n', () => {
    expect(() => buildDepositWithReceiptsTx(fullConfig, { ...validParams, amount: 0n })).toThrow(
      'must be > 0',
    );
  });
  ...
  it('rejects ratePerCall = 0n', () => {
    expect(() =>
      buildDepositWithReceiptsTx(fullConfig, { ...validParams, ratePerCall: 0n }),
    ).toThrow('must be > 0');
  });

  it('rejects feeMicroPercent = 1_000_001', () => {
    expect(() =>
      buildDepositWithReceiptsTx(fullConfig, { ...validParams, feeMicroPercent: 1_000_001 }),
    ).toThrow('feeMicroPercent');
  });

  it('rejects odd-length providerPubkey hex', () => {
    expect(() =>
      buildDepositWithReceiptsTx(fullConfig, { ...validParams, providerPubkey: 'abc' }),
    ).toThrow('odd length');
  });

  it('rejects non-hex providerPubkey', () => {
    expect(() =>
      buildDepositWithReceiptsTx(fullConfig, { ...validParams, providerPubkey: 'ZZZZ' }),
    ).toThrow('non-hex');
  });
});
...
describe('buildPrepaidTopUpTx validation', () => {
  ...
  it('FIXED F-07: validates amount = 0n (assertPositive added)', () => {
    expect(() => buildPrepaidTopUpTx(fullConfig, { ...validParams, amount: 0n })).toThrow(
      /must be > 0/,
    );
  });

  it('FIXED F-07: validates negative amount', () => {
    expect(() => buildPrepaidTopUpTx(fullConfig, { ...validParams, amount: -1n })).toThrow(
      /must be > 0/,
    );
  });
});
...
describe('buildMandatedPayTx validation', () => {
  ...
  it('validates feeMicroPercent', () => {
    expect(() =>
      buildMandatedPayTx(config, { ...validParams, amount: 100n, feeMicroPercent: -1 }),
    ).toThrow('feeMicroPercent');
  });
  ...
  it('FIXED F-08: validates amount = 0n (assertPositive added)', () => {
    expect(() =>
      buildMandatedPayTx(config, { ...validParams, amount: 0n, feeMicroPercent: 5_000 }),
    ).toThrow(/must be > 0/);
  });
});
...
describe('buildAgentMandatedPayTx validation', () => {
  ...
  it('validates feeMicroPercent', () => {
    expect(() =>
      buildAgentMandatedPayTx(config, { ...validParams, amount: 100n, feeMicroPercent: -1 }),
    ).toThrow('feeMicroPercent');
  });
  ...
  it('FIXED F-09: validates amount = 0n (assertPositive added)', () => {
    expect(() =>
      buildAgentMandatedPayTx(config, { ...validParams, amount: 0n, feeMicroPercent: 5_000 }),
    ).toThrow(/must be > 0/);
  });
});
...
describe('buildCreateMandateTx validation', () => {
  ...
  it('FINDING F-10: does NOT validate maxPerTx = 0n', () => {
    expect(() =>
      buildCreateMandateTx(config, { ...validParams, maxPerTx: 0n, maxTotal: 100n }),
    ).not.toThrow();
  });

  it('FINDING F-10: does NOT validate maxTotal = 0n', () => {
    expect(() =>
      buildCreateMandateTx(config, { ...validParams, maxPerTx: 100n, maxTotal: 0n }),
    ).not.toThrow();
  });
...
describe('buildCreateAgentMandateTx validation', () => {
  ...
  it('rejects L2+ mandate with maxTotal = 0n (v8 audit F-14)', () => {
    expect(() => buildCreateAgentMandateTx(config, { ...validParams, maxTotal: 0n })).toThrow(
      'zero lifetime budget',
    );
  });
  ...
  it('allows L0 with maxTotal = 0n', () => {
    expect(() =>
      buildCreateAgentMandateTx(config, {
        ...validParams,
...
describe('validation coverage summary', () => {
  ...
  it('buildDepositTx (prepaid) validates amount + rate + fee', () => {
    expect(() =>
      buildDepositTx(fullConfig, {
        coinType: SUI_COIN_TYPE,
        sender: SENDER,
        provider: RECIPIENT,
        amount: 0n,
        ratePerCall: 1000n,
        withdrawalDelayMs: 1800000n,
        feeMicroPercent: 5_000,
        feeRecipient: FEE_RECIPIENT,
      }),
    ).toThrow();
