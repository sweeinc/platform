...
import { createBuilderConfig, EscrowContract, PaymentContract, StreamContract } from '../src';
...
async function demoPayment(
  ...
  log(
    'pay',
    `Paying ${Number(PAYMENT_AMOUNT) / 1e9} SUI to recipient with ${FEE_MICRO_PCT / 10_000}% fee`,
  );
...
async function demoStream(
  ...
  log(
    'stream',
    `Creating stream: ${Number(STREAM_RATE)} MIST/sec, ${Number(STREAM_DEPOSIT) / 1e9} SUI deposit`,
  );
  ...
  const meterId =
    streamEvent?.parsedJson &&
    typeof streamEvent.parsedJson === 'object' &&
    'meter_id' in streamEvent.parsedJson
  ...
  if (BigInt(recipientBalance.totalBalance) > 1_000_000n) {
    ...
    try {
      ...
      if (
        claimEvent?.parsedJson &&
        typeof claimEvent.parsedJson === 'object' &&
        'amount' in claimEvent.parsedJson
      ) {
...
async function demoEscrow(
  ...
  if (
    escrowEvent?.parsedJson &&
    typeof escrowEvent.parsedJson === 'object' &&
    'escrow_id' in escrowEvent.parsedJson
  ) {
